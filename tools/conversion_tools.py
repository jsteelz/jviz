...
def convert_calendars(calendar_path, calendar_dates_path, out_dir):
  ...
  for date in service_jkeys_by_date:
    out_path = os.path.join(out_dir, date + '.json')
    with open(out_path, 'w') as out:
      json.dump(service_jkeys_by_date[date], out)
    ...
    for line in stops_reader:
      ...
      if (location_type == '' or location_type == '0'):
        try:
          ...
          print("Missing or badly formatted required value in stops.txt. Will not visualize.")
          exit(1)
  ...
  with open(trips_path, 'r', encoding='utf-8-sig') as trips:
    ...
    for line in trips_reader:
      ...
      if route_jkey in routes_obj:
        trips_obj[trip_jkey] = {'route_jkey': route_jkey, 'service_jkey': service_jkey, 'trip_id': trip_id, 'trip_jkey': trip_jkey}
...
def process_trips(trips_obj, stops_obj):
  ...
  for trip_jkey in trips_obj:
    ...
    if not (re.match(gtfs_time_regex, first_stop_time) and re.match(gtfs_time_regex, last_stop_time)):
    ...
    for stop_time in trips_obj[trip_jkey]['stop_times']:
      ...
      if this_time:
        try:
          ...
          hh, mm, ss = int(time_matched.group(1)), int(time_matched.group(2)), int(time_matched.group(3))
      ...
      tuple_to_append = (stop_id, stops_obj[stop_id]['stop_lat'], stops_obj[stop_id]['stop_lon'])
    ...
    if not itinerary_id in itin_obj:
      ...
      itin_name = first_stop_name + ' → ' + last_stop_name + ', ' + str(len(stop_list)) + ' stops'
...
def convert_trips(stops_path, trips_path, stop_times_path, stops_dir, trips_dir, itin_dir, trips_hour_dir, routes_dir, routes_obj):
  ...
  [trips_obj, trip_jkey_by_trip_id] = read_trips(trips_path, trips_dir, routes_obj)

  read_stop_times(stop_times_path, trips_obj, stops_obj)

  [itin_obj, trips_hour_obj, itins_with_sample_trips_by_route] = process_trips(trips_obj, stops_obj)

  for route_jkey in itins_with_sample_trips_by_route:
    route_path = os.path.join(routes_dir, route_jkey + '.json')
    with open(route_path, 'r') as f:
      route = json.load(f)
      all_trips = itins_with_sample_trips_by_route[route_jkey].values()
      route['sample_trip_jkeys'] = [item for sublist in all_trips for item in sublist]
    
    with open(route_path, 'w') as out:
      json.dump(route, out)

  for trip_jkey in trips_obj:
    out_path = os.path.join(trips_dir, trip_jkey + '.json')
    with open(out_path, 'w') as out:
      json.dump(trips_obj[trip_jkey], out)
  
  for itin_id in itin_obj:
    route_jkey = itin_id.split(itin_separator)[0]
    out_dir = os.path.join(itin_dir, route_jkey)
    if not os.path.isdir(out_dir):
      os.mkdir(out_dir)
    
    out_path = os.path.join(out_dir, 'itin_' + itin_id + '.json')
    with open(out_path, 'w') as out:
      json.dump(itin_obj[itin_id], out)
  
  for service_jkey_route_jkey in trips_hour_obj:
    out_path = os.path.join(trips_hour_dir, service_jkey_route_jkey + '.json')
    with open(out_path, 'w') as out:
      json.dump(trips_hour_obj[service_jkey_route_jkey], out)
