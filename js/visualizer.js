...
const map = new mapboxgl.Map({
  ...
  style: 'mapbox://styles/jsteele28/ckeskqyyo85xz19qm6lndlf5i', // Transit-streets
...
function routeClicked(routeJkey, routeShortName) {
  ...
  if (!/\d{8}/.test(selectedDate) && !/\d\d:\d\d/i.test(selectedTime)) {
    ...
    loadJsonFileAsObj(`./.visualizefiles/service_jkeys_by_date/${dateToUse}.json`, (serviceJkeys) => {
      getAllTripsAtTime(serviceJkeys, routeJkey, timeToUse);
    }, () => {
...
function getAllTripsAtTime(serviceJkeys, routeJkey, timeToUse) {
  let trips = [];
  let processed = 0;
  serviceJkeys.forEach((serviceJkey) => {
    const searchKey = `${routeJkey}_${serviceJkey}`;
    loadJsonFileAsObj(`./.visualizefiles/trips_by_route_by_hour/${searchKey}.json`, (tripsByHour) => {
      const hour = parseInt(timeToUse.slice(0, -3));
      if (tripsByHour[hour]) {
        trips = trips.concat(tripsByHour[hour]);
      }
      processed += 1;
      if (processed === serviceJkeys.length) {
        groupTripsByItineraryAndDisplay(trips, routeJkey);
      }
    }, () => {
      console.log(`No trips found for route ${routeJkey} on service id ${serviceJkey}.`);
      processed += 1;
      if (processed === serviceJkeys.length) {
        groupTripsByItineraryAndDisplay(trips, routeJkey);
      }
